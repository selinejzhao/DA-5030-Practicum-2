...
Let's air on the side of being cautious with removing outliers, since we don't want to risk information loss. Therefore, we are going to remove the outliers that the z-score method found. These are Observation numbers 94, 40, and 60. 
...
First before building any regression model we have to check if our features are normally distributed. Regression models make the assumption that features are normally distributed. If they are not, we can try some transforms to change the distribution. 
...
```
It looks like there is a bit of a right skew here but not too bad. We are going to leave it.
...
#replace sale price with its inverse
uffi.norm <- uffi.no
...
Enc_Pk_Spaces is a categorical, non-binary variable, but it is ordinal so we don't need to dummy code. You do not need to dummy code numerical ordinal variables such as Year_Sold and Enclosed Parking Spaces
...
Let's see if we can improve our model by incorporating any non-linear relationships. Let's plot different variables against Sale_Price to see if we can visually detect non-linear relationships.

```{r}
#plot continuous variables against Sale Price
plot(uffi.norm$Year_Sold, uffi.norm$Sale_Price)
plot(uffi.norm$Bsmnt_Fin_SF, uffi.norm$Sale_Price)
plot(uffi.norm$Living_Area_SF, uffi.norm$Sale_Price)
plot(uffi.norm$Lot_Area, uffi.norm$Sale_Price)
```
For Basement Square Feet it looks like the data could be clustered if we desired, since there are clear distinctions between howuses with and without basements. Separate regression equations could be useful in this instance to have different regression components based on if a home had a basement or not. 

The only other thing of not is in the Living area plot. It seems like an exponential cureve could possible fit the data better than the linear regression, but the difference isn't visually noticeable enough to add this exponential component to the model. We are going to leave the model as is for now with the cauveat of there is likely a clustering and/or non-linear component that could be added to slightly improve the model. However, the need wasn'tobvious enough. 

Everything else looks fine with our linear fitting. 
...
We are also going to run a one-way ANOVA (which is the same as a t-test for only 2 samples) to compare the means of the sales prices of the homes with UFFI present and the homes with UFFI absent. Since our sample size for UFFI present homes is less than 30 , we can't use a z-test to test for statistical significance in the delta between the means. 

For the t-test we are assuming that the Null hypothesis is that the means of the UFFI absent and present homes are the same. The Alternative hypothesis is that they are different. 
...
#print the mean and standard deviation for the two samples: ne with uffi presence & sale price, and one with uffi absence and sale price
group_by(uffi.no, as.factor(uffi.norm$UFFI_IN)) %>%
  summarise(
    count = n(),
...
```
As we can see by this tibble (data frame), the means and standard deviations of the two samples are listed. They do vary quite a bit, but we have to test to see if this variance is statistically significant. 

Noe let's visualize how the two samples compare. 
```{r}
#plot the two samples to visualize
ggboxplot(uffi.norm, x = "UFFI_IN", y = "Sale_Price", 
          color = "UFFI_IN", palette = c("#00AFBB", "#E7B800"),
          ylab = "Sale Price", xlab = "UFFI Presence")
```
Visually we can see that it looks like the presence of UFFI does indeed lower the price of a home. We still have to see if it is statistically significant. 

Now let's make a different type of plot: a mean plot. 

```{r}
#show a mean plot
ggline(uffi.norm, x = "UFFI_IN", y = "Sale_Price", 
       add = c("mean_se", "jitter"),
       ylab = "Sale Price", xlab = "UFFI Presence")
```
Here we can see in another wat the differences in the mean sale price between the two groups. Based on the downward slope of the line connecting the means, we can infer that there is a difference. Now let's see if it is statistically significant. 

Time to compute the ANOVA for 2 samples/t-test.

```{r}
# Compute the analysis of variance
anova.t <- aov(Sale_Price ~ UFFI_IN, data = uffi.norm)
# Summary of the analysis
summary(anova.t)
```
Since the p-value here is .0396, which is less than our .05 threshold, we can conclude further that the differences in Sale_Price dependent on the presence of UFFI in a home IS significant. This stretngthens our case the our client is justified in receiving restitution for the presence of UFFI not being disclosed to them. 

To show that this is the same as a t-test for 2 samples, we're going to run a t-test. 

```{r}
#run t test comparing presence and absence of UFFI on sale price
ttest <- t.test(Sale_Price ~ UFFI_IN, data = uffi.norm, var.equal = TRUE)
ttest
```
With a t value of 2.0874 and a p-value of .03956 (which is <.05), we reach the same conclusion as before. There is a statistically significant difference in the means. 
...
[3] 	STHDA, "One-Way ANOVA Test in R," Statistical tools for high-throughput data analysis, [Online]. Available: http://www.sthda.com/english/wiki/one-way-anova-test-in-r. [Accessed 5 November 2018].
...
Here's a walkthrough using the customer data from Assignment 1: Disclaimer, not all necessary data-preprocessing was completed for kNN. The imputed values are likely not accurate because we did not normalize the data. However, the purpose in doing a walkthrough is to show how kNN for imputation can work in practice, not to actually use the imputed values and dataset.
